...
const HtmlPart = ({isOpen, setIsOpen}) => {
  return (
    ...
    variants={variants}>
        ...
        <motion.div variants={simpleAnimation} initial="hidden" animate="show"  className="explore absolute top-0 right-0  p-2 sm:p-6 md:p-8 w-fit flex flex-col  gap-2 sm:text-sm text-xs">
        ...
        <motion.div variants={simpleAnimation} initial="hidden" animate="show" className="absolute left-0  bottom-0 p-2 sm:p-6 md:p-8  w-fit ">
            ...
            <div className="explore w-fit">
                <motion.h3 onClick={()=>{setIsOpen(false)}}  className="text-sm md:text-base lg:text-lg w-fit py-2">Find Out More & Get in Touch</motion.h3>
                <div className="line w-full h-[2px]  bg-[#3e3e3d]" />
            </div>
        </motion.div>
